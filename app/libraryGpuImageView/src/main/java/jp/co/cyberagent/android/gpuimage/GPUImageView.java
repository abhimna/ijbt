...
public class GPUImageView extends FrameLayout {
	...
	ShortBuffer sb;
	ShortBuffer sbt;
	private Bitmap totalSizebitmap;
	public static Bitmap result;

	public void setToatalSizeBitmap(Bitmap totalSizebitmap) {
		this.totalSizebitmap = totalSizebitmap;
	}
	...
	public void setOnPictureLoadListener(
			OnPictureLoadListener pictureLoadListener) {
		mGPUImage.setOnPictureLoadListener(pictureLoadListener);
	}
	...
	public Bitmap capture() throws InterruptedException {
		...
		/*
		 * final int width = mGLSurfaceView.getMeasuredWidth(); final int height
		 * = mGLSurfaceView.getMeasuredHeight(); sb = ShortBuffer.allocate
		 * (width * height); // 2 Bytes per-pixel sbt = ShortBuffer.allocate
		 * (width * height); // 2 Bytes per-pixel // Take picture on OpenGL
		 * thread // final int[] pixelMirroredArray = new int[width * height];
		 * mGPUImage.runOnGLThread(new Runnable() {
		 * 
		 * @Override public void run() { //final IntBuffer pixelBuffer =
		 * IntBuffer.allocate(width * height);
		 * 
		 * 
		 * GLES20.glReadPixels(0, 0, width, height, GLES20.GL_RGB,
		 * GLES20.GL_UNSIGNED_SHORT_5_6_5, sb); // int[] pixelArray =
		 * pixelBuffer.array();
		 * 
		 * // Convert upside down mirror-reversed image to right-side up normal
		 * image. for (int i = 0; i < height; i++) { for (int j = 0; j < width;
		 * j++) { sbt.put((height - i - 1) * width + j , sb.get (i * width +
		 * j)); } } waiter.release(); } }); requestRender(); waiter.acquire();
		 * 
		 * Bitmap bitmap = Bitmap.createBitmap(width, height,
		 * Bitmap.Config.RGB_565); bitmap.copyPixelsFromBuffer(sbt);
		 */
		...
		waiter.acquire();
		Bitmap bitmap;
		if (totalSizebitmap == null) {
			bitmap = Bitmap
					.createBitmap(width, height, Bitmap.Config.ARGB_8888);
		} else {
			bitmap = totalSizebitmap;
		}
	...
	private class SaveTask extends AsyncTask<Void, Void, Void> {
		...
		private void saveImage(final String folderName, final String fileName,
			...
			try {
				 ...
				 * 
				 * @Override public void onScanCompleted(final String path,
				 * final Uri uri) { if (mListener != null) { mHandler.post(new
				 * Runnable() {
				 * 
				 * @Override public void run() { mListener.onPictureSaved(uri);
				 * } }); } } });
				 */
				mListener.onPictureSaved(file);
