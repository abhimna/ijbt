...
public class GPUImageFilterTools {
	...
	private static GPUImageFilter createFilterForType(final Context context,
		...
		case LOOKUP_AMATORKA:
			...
			// amatorka.setBitmap(BitmapFactory.decodeResource(
			// context.getResources(), R.drawable.lookup_amatorka));
			return amatorka;
		default:
	...
	public static class FilterAdjuster {
		...
		private class ColorBalanceAdjuster extends
			...
			public void adjust(int percentage) {
				getFilter().setMidtones(
						new float[] { range(percentage, -0.3f, 0.3f),
								range(percentage, -0.0f, 0.0f),
								range(percentage, -0.0f, 0.0f) });
			}
		}
		...
		private abstract class Adjuster<T extends GPUImageFilter> {
			...
			protected int range(final int percentage, final int start,
					final int end) {
				return (end - start) * percentage / 200 + start;
			}
		}
		...
		private class SharpnessAdjuster extends Adjuster<GPUImageSharpenFilter> {
			@Override
			public void adjust(final int percentage) {
				getFilter().setSharpness(range(percentage, -0.0f, 1.0f));
			}
		}
		...
		private class ContrastAdjuster extends Adjuster<GPUImageContrastFilter> {
			@Override
			public void adjust(final int percentage) {
				getFilter().setContrast(range(percentage, 0.7f, 1.3f));
			}
		}
		...
		private class BrightnessAdjuster extends
			...
			@Override
			public void adjust(final int percentage) {
				getFilter().setBrightness(range(percentage, -0.1f, 0.1f));
			}
		}
